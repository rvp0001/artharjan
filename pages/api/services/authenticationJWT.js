...
const verifyMobileOTPJWT = async (_parent,userData, context) => {
...
const signInMobileJWT = async (_parent,userData, context) => {
...
const signUpMobileJWT = async (_parent,userData, //Input is user object and request
  context) => {
...
const signInUsernameJWT = async (_parent,userData, context) => {
  ...
  if (userCount == 0) {
    ...
    let validPass = await bcrypt.compare(password, userFound.password);
     console.log('^^^^^^^^^^--02validPass^^^^^^^^')
    if (validPass) {

      console.log('^^^^^^^^^^--03validPass^^^^^^^^')
      const token = generateTokenUser(userFound);
      console.log('^^^^^^^^^^--04validPass^^^^^^^^',userFound)
      return { ...userFound, token }
    }
    else {
...
const currentUserUsernameJWT = async (_parent,args, context) => {
...
const signUpUsernameJWT = async (_parent,userData, //Input is user object and request
  context) => {
